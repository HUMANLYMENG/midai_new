...
- **单张获取**：添加/编辑专辑时一键获取封面
- **共享缓存**：已获取的封面缓存到本地数据库，供所有用户共享使用

### 🎵 歌单导入
- **QQ 音乐**：支持解析 QQ 音乐歌单链接
- **网易云音乐**：支持解析网易云音乐歌单链接
- **自动获取流派**：导入时自动从 MusicBrainz 获取流派信息
- **重复检测**：自动跳过已存在的歌曲
...
### 🔐 用户认证
- **双模式认证**：开发环境免登录 / 生产环境强制 OAuth
- **Google OAuth**：支持 Google 账号登录
- **Microsoft OAuth**：支持 Microsoft 账号登录
- **用户数据隔离**：每个用户独立的专辑和单曲数据
...
├── components/
│   ├── album/
│   │   ├── CollectionList.tsx    # 侧边栏列表（虚拟滚动）
│   │   ├── CollectionForm.tsx    # 编辑/添加表单
│   │   ├── UnifiedImportModal.tsx # 统一导入（CSV + 链接）
...
├── lib/
│   ├── album-cache.ts            # 共享专辑缓存服务
│   ├── auth.ts                   # 认证工具（开发/生产模式）
...
│   └── utils.ts                  # 工具函数
├── middleware.ts                 # Next.js 路由中间件（认证）
├── prisma/
│   └── schema.prisma             # 数据库模型
├── types/
...
| 单曲 CRUD | ✅ | 创建、读取、更新、删除单曲 |
| CSV 批量导入 | ✅ | 支持编码检测、自动去重 |
| 力导向图谱 | ✅ | D3.js 实现，节点可拖拽 |
| 图谱交互 | ✅ | 悬停高亮、点击查看详情 |
| 专辑节点聚合 | ✅ | 点击专辑展开显示所属单曲 |
| 侧边栏列表 | ✅ | 排序、搜索、点击聚焦图谱节点 |
| 搜索过滤 | ✅ | 支持专辑/单曲/艺术家/流派搜索 |
| 深色/浅色主题 | ✅ | CSS Variables 实现 |
| 玻璃拟态 UI | ✅ | 毛玻璃效果、圆角、阴影 |
| 可拖拽弹窗 | ✅ | 编辑窗口可自由拖动 |
| 右键菜单 | ✅ | Focus / Edit / Delete / Fetch Cover 快捷操作 |
| **封面自动获取** | ✅ | 一键批量获取，Spotify 数据源，智能缓存 |
| 图谱高亮交互 | ✅ | 图例/流派/专辑/单曲单击高亮，背景取消 |
| 节点悬停效果 | ✅ | 放大、发光、边框增强 |
| 排序性能优化 | ✅ | 排序时不重新加载图谱 |
| **虚拟滚动** | ✅ | 上千首歌曲流畅浏览 |
| **共享缓存** | ✅ | 封面/流派缓存，所有用户共享 |
| **歌单导入** | ✅ | 支持 QQ音乐/网易云音乐 |
| **用户认证** | ✅ | 开发免登录/生产OAuth双模式 |
